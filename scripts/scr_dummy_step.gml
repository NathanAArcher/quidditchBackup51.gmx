...
if tackle == false and scuffling == false and ch_off_broom == false and stiffarmed == false and drag < 1 and argument0 == 0{
    if st_position == 4{ //if seeker; disregard all quaffle play
        ...
        if global.team2_score - global.team1_score >= -30{
            ...
            if(distance_to_point(global.seeker1x, global.seeker1y) < distance_to_point(obj_snitch.x, obj_snitch.y)){
                draw_direction = obj_snitch.direction;
            }
            if distance_to_point((global.seeker1x + obj_snitch.x*2)/3, (global.seeker1y + obj_snitch.y*2)/3) > 4{
                if distance_to_point((global.seeker1x + obj_snitch.x*2)/3, (global.seeker1y + obj_snitch.y*2)/3) > 16{
                    move_towards_point((global.seeker1x + obj_snitch.x*2)/3, (global.seeker1y + obj_snitch.y*2)/3, moving_speed)
                }else{
                    move_towards_point((global.seeker1x + obj_snitch.x*2)/3, (global.seeker1y + obj_snitch.y*2)/3, moving_speed-1)
                ...
                move_towards_point(global.seeker1x, global.seeker1y, 0)
            }
        }
        if(abs(direction - truedirection) >= 20){
            if (abs((truedirection + 15) - direction)) mod 360 < (abs((truedirection - 15) - direction)) mod 360{
        ...
        if st_position < 3{
            if go_broomsup == 0{
                if st_position != 2{
                    ...
                    if (global.threedmode == false and distance_to_point(obj_center_hoop.x, obj_center_hoop.y + po_keeper_distance) > 4) or (global.threedmode == true and distance_to_point(obj_center_hoop.x - po_keeper_distance, obj_center_hoop.y) > 4){
                        if global.threedmode == false{
                            move_towards_point(obj_center_hoop.x, obj_center_hoop.y + po_keeper_distance, moving_speed)
                        }else{
                            move_towards_point(obj_center_hoop.x - po_keeper_distance, obj_center_hoop.y, moving_speed)
                        }
                        thoughts = "Move to defensive position"
                    }else{
                        move_towards_point(global.ballX, global.ballY, 0)
                        thoughts = "Maintain defensive position " + string(obj_center_hoop.x) + ", " + string(obj_center_hoop.y-po_keeper_distance)
                    }
                }
            }
            if go_broomsup == 1{
            ...
            if st_position == 3{
                ...
                if ch_ball == false and instance_exists(obj_bludger){
                    ...
                    if(instance_number(obj_bludger) != 2){ //if 1 or 3 go to closest bludger
                        ...
                        if(scr_beater_dummy_is_bludgerless() and distance_to_point(scr_beater_dummy_nearest_me(x, y, "x"), scr_beater_dummy_nearest_me(x, y, "y") < 96)){ //if neither beaters have bludgers
                            //get_further_bludger_teammate returns a bludger's coordinates
                            if(instance_nearest((scr_get_further_bludger_dummy(x, y, "x")), ((scr_get_further_bludger_dummy(x, y, "y"))), obj_player_teammate).x == x
                            and instance_nearest((scr_get_further_bludger_dummy(x, y, "x")), ((scr_get_further_bludger_dummy(x, y, "y"))), obj_player_teammate).y == y){ //if I am closest teammate to further bludger
                                move_towards_point(scr_get_further_bludger_dummy(x, y, "x"), scr_get_further_bludger_dummy(x, y, "y"), moving_speed) //move towards further bludger
    ...
    if global.possession == 0{ //if quaffle free; if this causes an error scuffle does not do possesssion correctly
        if instance_exists(obj_quaffle){
            if st_position == 1{ //chaser (can use quaffle, doesn't have keeper privileges)
                if (scr_chaser_teammate_closest_quaffle("x") == x) and (scr_chaser_teammate_closest_quaffle("y") == y){ //should incorporate obj_player_test for other
                    if(abs(obj_quaffle.x - instance_nearest(obj_quaffle.x, obj_quaffle.y, obj_player_dummy).x)
                    + abs(obj_quaffle.y - instance_nearest(obj_quaffle.x, obj_quaffle.y, obj_player_dummy).y)) -
                    (abs(obj_quaffle.x - instance_nearest(obj_quaffle.x, obj_quaffle.y, obj_player_teammate).x) //also do obj_player_test
                    + abs(obj_quaffle.y - instance_nearest(obj_quaffle.x, obj_quaffle.y, obj_player_teammate).y)) < 32{ // if chance of getting it before opposing player
                    ...
                    if scr_team1_is_closest() == true and ((global.threedmode == false and global.ballY > room_height/2) or (global.threedmode == true and global.ballX < room_width/2)){
                        if distance_to_point(po_dx, po_dy) > 4{
                            move_towards_point(po_dx, po_dy, moving_speed)
                        ...
                        if distance_to_point(po_ox, po_oy) > 16{
                            ...
                            if(scr_chaser_dummy_closest_quaffle("x") == x and scr_chaser_dummy_closest_quaffle("y") == y){
                                move_towards_point(global.ballX, global.ballY, moving_speed)
                                sprint = 1
        ...
        if st_position == 2{ //keeper, like a chaser but can DIVE in keeper zone
            if (global.threedmode == false and global.ballY < obj_center_hoop.y and y > obj_center_hoop.y + po_keeper_distance) or (global.threedmode == true and global.ballX > obj_center_hoop.x and x < obj_center_hoop.x - po_keeper_distance){
                ...
                if (global.threedmode == false and distance_to_point(obj_center_hoop.x, obj_center_hoop.y + po_keeper_distance) > 4) or (global.threedmode == true and distance_to_point(obj_center_hoop.x - po_keeper_distance, obj_center_hoop.y) > 4){
                    ...
                    move_towards_point(global.ballX, global.ballY, 0)
                    thoughts = "In front of hoops; stare at ball"
                }
            }
            if (global.threedmode == false and global.ballY > obj_center_hoop.y and y < obj_center_hoop.y - po_keeper_distance) or (global.threedmode == true and global.ballX < obj_center_hoop.x and x > obj_center_hoop.x + po_keeper_distance){
                ...
                move_towards_point(global.ballX, global.ballY, 0)
                thoughts = "Behind hoops; stare at ball"
                }
            }
            if abs(((instance_nearest(obj_quaffle.x, obj_quaffle.y, obj_player_teammate).x - obj_quaffle.x)) + abs((instance_nearest(obj_quaffle.x, obj_quaffle.y, obj_player_teammate).x - obj_quaffle.x)) + 32) < (abs(x - obj_quaffle.x) + abs(y - obj_quaffle.y)){
                //if closer than opponent by 32
                thoughts = "Closer than opponents; move quaffle";
                move_towards_point(obj_quaffle.x, obj_quaffle.y-12, moving_speed)
                }
            
        if (distance_to_object(obj_center_hoop) > (po_keeper_distance + 8) and distance_to_object(obj_baby_hoop) > (po_keeper_distance + 8) and distance_to_object(obj_smedium_hoop) > (po_keeper_distance + 8)){
            thoughts = "Far enough; move quaffle" + string(distance_to_object(obj_center_hoop));
            jumping = false
            move_towards_point(obj_quaffle.x, obj_quaffle.y-12, moving_speed)
            }else{
                thoughts = "Dive?"
                if dive_cooldown == 0{
                    if round(random_range(1, 6)) == 1 and jumping == false and obj_quaffle.speed > 3 and distance_to_object(obj_quaffle) < 160{
                        thoughts = "Dive!"
                        if global.threedmode == false{
                            if abs(global.marker1x-obj_center_hoop.x) > 32{
                                jumping = true
                            }
                            if global.marker1x > obj_center_hoop.x{
                                jumpX = obj_smedium_hoop.x
                                jumpY = y
                                dive_cooldown = 30
                            }
                            if global.marker1x > obj_center_hoop.x == false{
                                jumpX = obj_baby_hoop.x
                                jumpY = y
                                dive_cooldown = 30
                                }
                        }else{
                            if abs(global.marker1y-obj_center_hoop.y) > 32{
                                jumping = true
                            }
                            if global.marker1y > obj_center_hoop.y{
                                jumpY = obj_smedium_hoop.y
                                jumpX = x
                                dive_cooldown = 30
                            }
                            if global.marker1y > obj_center_hoop.y == false{
                                jumpY = obj_baby_hoop.y
                                jumpX = x
                                dive_cooldown = 30
                                }                            
                            }
                ...
                if jumping == true{
                    move_towards_point(jumpX, jumpY, st_power + 5)
                    thoughts = "JUMP"
                    if distance_to_point(jumpX, jumpY) > 6{
                        jumping = false
                    }
                }
                if (distance_to_point(obj_quaffle.x, obj_quaffle.y) < scr_chaser_teammate_closest_quaffle("distance")) and (distance_to_point(obj_center_hoop.x, obj_center_hoop.y) - po_keeper_distance) > 8{
                    sprint = 1
                    move_towards_point(obj_quaffle.x, obj_quaffle.y-12, moving_speed)
          ...
          if st_position < 3 and instance_exists(obj_passmarker){ //if pass to me, disregard all else //and obj_quaffle.pass == 2 removed
            if distance_to_point(obj_passmarker.x, obj_passmarker.y) < 48{ //originally global.marker1x, global.marker1y
                ...
                scr_chaser_dummy_closest_pass("x") == x and scr_chaser_dummy_closest_pass("y") == y and throw_leway == 0{
                    ...
                    sprint = 1
                    move_towards_point(global.ballX, global.ballY, moving_speed)
                    if distance_to_point(global.ballX, global.ballY) <= 48 and obj_quaffle.pass == 2{ //if close to ball, move closer to ball
                        move_towards_point(global.ballX, global.ballY, moving_speed)
          ...
          if st_position < 3 and (instance_nearest(global.ballX, global.ballY, obj_player_dummy).x == x and instance_nearest(global.ballX, global.ballY, obj_player_dummy).y == y){
                sprint = 1
          ...
          if st_position == 3{
            if instance_exists(obj_bludger){ //if true is covered later down
                if ch_ball == false{
                    ...
                    if ch_ball == true and scr_beater_dummy_is_bludgerless() and
                    distance_to_point(scr_beater_dummy_nearest_me(x, y, "x"), scr_beater_dummy_nearest_me(x, y, "y")) > distance_to_object(instance_nearest(x, y, obj_bludger)){
                        move_towards_point((instance_nearest(x, y, obj_bludger)).x, (instance_nearest(x, y, obj_bludger)).y, moving_speed)
                        ...
                        if distance_to_point((instance_nearest(x, y, obj_bludger)).x, (instance_nearest(x, y, obj_bludger)).y) < 96{
                            ...
                            if round(random_range (1, 8)) == 1{
                                ...
                                throw_leway = 15
                                if(distance_to_point(scr_eligible_beater_dummy_location("x"), scr_eligible_beater_dummy_location("y")) < distance_to_point(obj_baby_score.x, obj_baby_score.y)+16){
                                    global.targetX = scr_eligible_beater_dummy_location("x")
                                    global.targetY = scr_eligible_beater_dummy_location("y")
                                    global.targetSpeed = 0
                                    if(global.threedmode == true){
                                        global.targetX = obj_baby_score.x
                                    }else{
                                        global.targetY = obj_baby_score.y
                                    }
                                    if(scr_is_target_dummy_beat(global.targetX, global.targetY)){ //he's beat, don't pass to him, throw to hoops
                                        scr_throw_ball(obj_bludger, x, y, 14, 0) //13 or 14 depending on team
                                        ch_ball = false
                                        exit
                                    }
                                    scr_throw_ball(obj_bludger, x, y, 9, 0) //7 or 9 depending on team
                                    ch_ball = false
                                }else{
                                    //throws toward center hoop
                                    scr_throw_ball(obj_bludger, x, y, 14, 0) //13 or 14 depending on team
                                    ch_ball = false
                                }
                                exit
                    ...
                    if distance_to_object(obj_center_hoop) > 48{
                        move_towards_point(obj_center_hoop.x, obj_center_hoop.y+48, moving_speed)
                        thoughts = "Move below center hoop"
                    }else{
                        move_towards_point(global.ballX, global.ballY, 0)
    ...
    if global.possession == 1{ //if other team has quaffle
        if st_position < 4{
            if global.threedmode == true{
                if(x < global.ballX){
        ...
        if st_position == 1{
            ...
            if distance_to_point(global.ballX, global.ballY) > 24 and ((global.threedmode == false and y > po_chaser_zone) or (global.threedmode == true and x < po_chaser_zone)){
                    ...
                    if global.threedmode == false{
                        if manStrategy == 0{
                            ...
                            if global.ballY < po_chaser_zone{
                                ...
                                if scr_ballwrapped() != 0 and drag != 1{ //if the ball is wrapped and I am not the one wrapping!
                                    if distance_to_point((instance_nearest(x, y, obj_baby_score).x+global.ballX)/2, (instance_nearest(x, y, obj_baby_score).y+global.ballY)/2) > 8{
                                        move_towards_point((instance_nearest(x, y, obj_baby_score).x+global.ballX)/2, (instance_nearest(x, y, obj_baby_score).y+global.ballY)/2, moving_speed)
                        ...
                        if manStrategy == 0{
                            ...
                            if global.ballX > po_chaser_zone{
                                move_towards_point(global.ballX, global.ballY, moving_speed)
                                sprint = 1
                                thoughts = "Past center line; attack 2!"
                                if scr_ballwrapped() != 0 and drag != 1{ //if the ball is wrapped and I am not the one wrapping!
                                    if distance_to_point((instance_nearest(x, y, obj_baby_score).x+global.ballX)/2, (instance_nearest(x, y, obj_baby_score).y+global.ballY)/2) > 4{
                                        move_towards_point((instance_nearest(x, y, obj_baby_score).x+global.ballX)/2, (instance_nearest(x, y, obj_baby_score).y+global.ballY)/2, moving_speed)
                ...
                if defense == 1{
                    ...
                    if(distance_to_point(global.ballX, global.ballY) < po_radius){
                        ...
                        if(point_distance(po_dx, po_dy, global.ballX, global.ballY) < po_radius*1.5 and distance_to_point(global.ballX, global.ballY) < po_radius*.5
                            or ((instance_nearest(global.ballX, global.ballY, obj_player_dummy).x == x and instance_nearest(global.ballX, global.ballY, obj_player_dummy).y == y) and
                            point_distance(obj_baby_score.x, obj_baby_score.y, global.ballX, global.ballY) < 144))
                ...
                if defense == 0 and manStrategy == 0{ //man-to-man defense
                    if (global.threedmode == false and abs(y-po_chaser_zone) < 8 and global.ballY < po_chaser_zone == false) or (global.threedmode == true and abs(x-po_chaser_zone) < 8 and global.ballX > po_chaser_zone){
                        if (global.threedmode == false and abs(x-global.ballX) > 32) or (global.threedmode == true and (abs(y-global.ballY) > 32)){
                            ...
                            if scr_getKeeperBall() and ((global.threedmode == true and global.ballX < (room_width/2)-32) or (global.threedmode == false and global.ballY > (room_height/2)+32) or (instance_nearest(global.ballX, global.ballY, obj_player_teammate).immunitycount > 0)){
                                if distance_to_point(po_dx, po_dy) > 4{
                                    move_towards_point(po_dx, po_dy, moving_speed)
                                }else{
                                    move_towards_point(global.ballX, global.ballY, 0)
                ...
                if defense == 0 and manStrategy == 1 or manStrategy == 2{
                    ...
                    if defense == 1{
                        if distance_to_point(global.ballX, global.ballY) < po_radius and ((global.threedmode == false and global.ballY > po_chaser_zone == false) or (global.threedmode == true and global.ballX > po_chaser_zone == true)){ //if ball within radius and past halfway line
                        move_towards_point(global.ballX, global.ballY, moving_speed) //if nothing else; bug patch for not doing anything
                        thoughts = "Sprint on ball." //48, 2 lines above, should be po_radius
                        sprint = 1
                        if scr_ballwrapped() != 0 and drag != 1{ //if the ball is wrapped and I am not the one wrapping!
                                if distance_to_point((instance_nearest(x, y, obj_baby_score).x+global.ballX)/2, (instance_nearest(x, y, obj_baby_score).y+global.ballY)/2) > 4{
                                    move_towards_point((instance_nearest(x, y, obj_baby_score).x+global.ballX)/2, (instance_nearest(x, y, obj_baby_score).y+global.ballY)/2, moving_speed)
                            ...
                            if distance_to_point(po_dx, po_dy) > 8{
                                if distance_to_object(obj_baby_score) > 4{
                                    move_towards_point(po_dx, po_dy, moving_speed)
        ...
        if st_position == 2{
                if global.ballY < obj_center_hoop.y{
                    if distance_to_point(obj_center_hoop.x, obj_center_hoop.y-po_keeper_distance) > 8{
                        move_towards_point(obj_center_hoop.x, obj_center_hoop.y-po_keeper_distance, moving_speed)
                        thoughts = "Move behind center hoop"
                        }
                        if distance_to_point(obj_center_hoop.x, obj_center_hoop.y-po_keeper_distance) < 4{
                            move_towards_point(global.ballX, global.ballY, 0)
                            thoughts = "Stare at ball."                        
                        }
                    }  
            ...
            if distance_to_point(global.ballX, global.ballY) < po_radius{
                if scr_getKeeperBall() and ((global.threedmode == true and global.ballX < (room_width/2)-32) or (global.threedmode == false and global.ballY > (room_height/2)+32)){
                    ...
                    sprint = 1
                    if y+8 < global.ballY and global.ballY < obj_center_hoop.y{ //Move body inbetween hoops and ball
                        move_towards_point(global.ballX, global.ballY-16, moving_speed)
                    }else{
                        if y-8 > global.ballY and global.ballY > obj_center_hoop.y{
                            move_towards_point(global.ballX, global.ballY+16, moving_speed)
                        }else{
                            move_towards_point(global.ballX, global.ballY, moving_speed)
                            }
            ...
            if st_position == 3{
                ...
                if st_position == 3 and instance_exists(obj_bludger){
                    ...
                    if ch_ball == true and scr_beater_dummy_is_bludgerless() and ((global.threedmode == true and global.ballX < room_width/2-64) or (global.threedmode == false and global.ballY > room_height/2+64)){
                        move_towards_point((instance_nearest(x, y, obj_bludger)).x, (instance_nearest(x, y, obj_bludger)).y, moving_speed)
                        thoughts = "Retrieve bludger for beater buddy"
                        if distance_to_point((instance_nearest(x, y, obj_bludger)).x, (instance_nearest(x, y, obj_bludger)).y) < 96{
                            //throw bludger towards hoops
                            ...
                            if round(random_range (1, 8)) == 1{
                                ...
                                scr_throw_ball(obj_bludger, x, y, 14, 0)
                                exit
                            }
                        }                
                    }
    ...
    if global.possession == 2 and st_position < 3{ //If quaffle in possession and chaser/keeper
        if ch_ball == true{
                if hoopchoice == 0{
                ...
                if global.threedmode == true{
                    if hoopchoice == 1{
                        if distance_to_point(obj_baby_opp.x-48, obj_baby_opp.y) > 4{
                            move_towards_point(obj_baby_opp.x-48, obj_baby_opp.y, moving_speed)
                        }else{
                            move_towards_point(obj_baby_opp.x-48, obj_baby_opp.y, 0)
                        }
                    }
                    if hoopchoice == 2{
                        if distance_to_point(obj_center_opp.x-48, obj_center_opp.y) > 4{
                            move_towards_point(obj_center_opp.x-48, obj_center_opp.y, moving_speed)
                        }else{
                            move_towards_point(obj_center_opp.x-48, obj_center_opp.y, 0)
                        }
                    }
                    if hoopchoice == 3{
                        if distance_to_point(obj_smedium_opp.x-48, obj_smedium_opp.y) > 4{
                            move_towards_point(obj_smedium_opp.x-48, obj_smedium_opp.y, moving_speed)
                        }else{
                            move_towards_point(obj_smedium_opp.x-48, obj_smedium_opp.y, 0)
                        }
                    }
                }else{
                    if hoopchoice == 1{
                        if distance_to_point(obj_baby_opp.x, obj_baby_opp.y-8) > 4{
                            move_towards_point(obj_baby_opp.x, obj_baby_opp.y-8, moving_speed)
                        }else{
                            move_towards_point(obj_baby_opp.x, obj_baby_opp.y-8, 0)
                        }
                    }
                    if hoopchoice == 2{
                        if distance_to_point(obj_center_opp.x, obj_center_opp.y-8) > 4{
                            move_towards_point(obj_center_opp.x, obj_center_opp.y-8, moving_speed)
                        }else{
                            move_towards_point(obj_center_opp.x, obj_center_opp.y-8, 0)
                        }
                    }
                    if hoopchoice == 3{
                        if distance_to_point(obj_smedium_opp.x, obj_smedium_opp.y-8) > 4{
                            move_towards_point(obj_smedium_opp.x, obj_smedium_opp.y-8, moving_speed)
                        }else{
                            move_towards_point(obj_smedium_opp.x, obj_smedium_opp.y-8, 0)
                        }
                    }                
                }
            if distance_to_point(global.ballX, global.ballY) < 64 and global.possession == 1{
                move_towards_point(global.ballX, global.ballY, moving_speed)
                //original code for moving on player removed
            }
            if distance_to_object(obj_baby_score_opp) < 160{
                ...
                if round(random_range(1, 8)) == 1{ //take a shot if decide to
                    ...
                    if distance_to_object(obj_baby_score) < 48{
                        if global.threedmode == true{
                            if (abs(obj_baby_opp.y-y) < 24){
                                hoopchoice = 1
                            }
                            if (abs(obj_center_opp.y-y) < 24){
                                hoopchoice = 2
                            }
                            if (abs(obj_smedium_opp.y-y) < 24){
                                hoopchoice = 3
                            }
                        }else{
                            if (abs(obj_baby_opp.x-x) < 24){
                                hoopchoice = 1
                            }
                            if (abs(obj_center_opp.x-x) < 24){
                                hoopchoice = 2
                            }
                            if (abs(obj_smedium_opp.x-x) < 24){
                                hoopchoice = 3
                            }                        
                        }
                    }
                    ...
                    if global.threedmode == true{ //if a bad angle, don't throw!
                        if hoopchoice == 1{ //baby hoop
                            if ((abs(obj_baby_opp.x-x)) < (abs(obj_baby_opp.y-y))){
                                exit
                            }
                        }
                        if hoopchoice == 2{ //baby hoop
                            if ((abs(obj_center_opp.x-x)) < (abs(obj_center_opp.y-y))){
                                exit
                            }
                        }
                        if hoopchoice == 3{ //baby hoop
                            if ((abs(obj_smedium_opp.x-x)) < (abs(obj_smedium_opp.y-y))){
                                exit
                            }
                        }
                    }else{
                        if hoopchoice == 1{ //baby hoop
                            if ((abs(obj_baby_opp.y-y)) < (abs(obj_baby_opp.x-x))){
                                exit
                            }
                        }
                        if hoopchoice == 2{ //baby hoop
                            if ((abs(obj_center_opp.y-y)) < (abs(obj_center_opp.x-x))){
                                exit
                            }
                        }
                        if hoopchoice == 3{ //baby hoop
                            if ((abs(obj_smedium_opp.y-y)) < (abs(obj_smedium_opp.x-x))){
                                exit
                            }
                        }                    
                    }
                    if(global.threedmode == true){ //Don't throw if at bad angle, exit event
                        if(hoopchoice == 1){
                            if((point_direction(x,y,obj_baby_opp.x,obj_baby_opp.y) > 72 and (point_direction(x,y,obj_baby_opp.x,obj_baby_opp.y)) < 108))
                            or ((point_direction(x,y,obj_baby_opp.x,obj_baby_opp.y) > 252 and (point_direction(x,y,obj_baby_opp.x,obj_baby_opp.y)) < 288)){ //bad angle
                                thoughts = "Bad angle for shot."
                                exit
                            }
                        }
                        if(hoopchoice == 2){
                            if((point_direction(x,y,obj_center_opp.x,obj_center_opp.y) > 72 and (point_direction(x,y,obj_center_opp.x,obj_center_opp.y)) < 108))
                            or ((point_direction(x,y,obj_center_opp.x,obj_center_opp.y) > 252 and (point_direction(x,y,obj_center_opp.x,obj_center_opp.y)) < 288)){ //bad angle
                                thoughts = "Bad angle for shot."
                                exit
                            }
                        }
                        if(hoopchoice == 3){
                            if((point_direction(x,y,obj_smedium_opp.x,obj_smedium_opp.y) > 72 and (point_direction(x,y,obj_smedium_opp.x,obj_smedium_opp.y)) < 108))
                            or ((point_direction(x,y,obj_smedium_opp.x,obj_smedium_opp.y) > 252 and (point_direction(x,y,obj_smedium_opp.x,obj_smedium_opp.y)) < 288)){ //bad angle
                                thoughts = "Bad angle for shot."
                                exit
                            }                    
                        }
                        ...
                        if(hoopchoice == 1){
                            if((point_direction(x,y,obj_baby_opp.x,obj_baby_opp.y) > 162 and (point_direction(x,y,obj_baby_opp.x,obj_baby_opp.y)) < 198))
                            or ((point_direction(x,y,obj_baby_opp.x,obj_baby_opp.y) > 342 or (point_direction(x,y,obj_baby_opp.x,obj_baby_opp.y)) < 18)){ //bad angle
                                thoughts = "Bad angle for shot."
                                exit
                            }
                        }
                        if(hoopchoice == 2){
                            if((point_direction(x,y,obj_center_opp.x,obj_center_opp.y) > 162 and (point_direction(x,y,obj_center_opp.x,obj_center_opp.y)) < 198))
                            or ((point_direction(x,y,obj_center_opp.x,obj_center_opp.y) > 342 or (point_direction(x,y,obj_center_opp.x,obj_center_opp.y)) < 18)){ //bad angle
                                thoughts = "Bad angle for shot."
                                exit
                            }
                        }
                        if(hoopchoice == 3){
                            if((point_direction(x,y,obj_smedium_opp.x,obj_smedium_opp.y) > 162 and (point_direction(x,y,obj_smedium_opp.x,obj_smedium_opp.y)) < 198))
                            or ((point_direction(x,y,obj_smedium_opp.x,obj_smedium_opp.y) > 342 or (point_direction(x,y,obj_smedium_opp.x,obj_smedium_opp.y)) < 18)){ //bad angle
                                thoughts = "Bad angle for shot."
                                exit
                            }                    
                        }                    
            ...
            if distance_to_object(obj_baby_score_opp) < 32{
                ...
                thoughts = "Lay up"
                if distance_to_point(obj_baby_opp.x, obj_baby_opp.y) < 32{
                    hoopchoice = 1
                }
                if distance_to_point(obj_center_opp.x, obj_center_opp.y) < 32{
                    hoopchoice = 2
                }
                if distance_to_point(obj_smedium_opp.x, obj_smedium_opp.y) < 32{
                    hoopchoice = 3
                }
                if(global.threedmode == true){ //Don't throw if at bad angle, exit event
                    if(hoopchoice == 1){
            ...
            if distance_to_object(obj_player_teammate) < 24 and instance_nearest(x, y, obj_player_teammate).st_position < 4
            and instance_nearest(x, y, obj_player_teammate).tackle == false and instance_nearest(x, y, obj_player_teammate).ch_off_broom == false
            and immunitycount == 0{ //threatened by opposing player
                ...
                if advancechoice = -1{ //reset by throwing backwards
                    ch_ball = false
                    scr_throw_ball(obj_quaffle, x, y, 6, 0)
                    advancechoice = 0
                    exit
                }
                ...
                thoughts = "Checking passing option"
                    with(obj_player_dummy){
                        scr_check_eligible_dummy() //sets global.targetX and global.targetY to a dummy if eligible
                    }
                    if (obj_player_dummy.x == global.targetX) and (obj_player_dummy.y  == global.targetY)  and (global.targetX != x or global.targetY != y){
            ...
            if distance_to_object(obj_player_teammate) < 48 and instance_nearest(x, y, obj_player_teammate).st_position == 3 and instance_nearest(x, y, obj_player_teammate).tackle == false and instance_nearest(x, y, obj_player_teammate).ch_ball == true{
                if advancechoice = 0{ //charge
                    if random(10) > 9{
                        advancechoice = irandom(2)
                    }
                ...
                if advancechoice == 2{ //pass to player
                    with(obj_player_dummy){
            ...
            if(st_position == 2){ //if keeper
                if(global.threedmode){
                    if((instance_nearest(x, y, obj_player_teammate).x > room_width/2+64) and instance_nearest(x, y, obj_player_teammate).ch_off_broom == false){
                        if(distance_to_point(scr_eligible_beater_dummy_location("x"), scr_eligible_beater_dummy_location("y")) > distance_to_point(instance_nearest(x, y, obj_player_teammate).x, instance_nearest(x, y, obj_player_teammate).y)){
                            if(distance_to_point(1200, y) > 8){
                                move_towards_point(1200, y, moving_speed)
                    ...
                    if((instance_nearest(x, y, obj_player_teammate).y < room_height/2-48) and instance_nearest(x, y, obj_player_teammate).ch_off_broom == false){
                        if(distance_to_point(scr_eligible_beater_dummy_location("x"), scr_eligible_beater_dummy_location("y")) > distance_to_point(instance_nearest(x, y, obj_player_teammate).x, instance_nearest(x, y, obj_player_teammate).y)){
                            if(distance_to_point(x, 224) > 8){
                                move_towards_point(x, 224, moving_speed)
                                thoughts = "Danger. Move to keeper zone."
                            }else{
            ...
            if distance_to_point(po_ox, po_oy) > 24{ //and thoughts != "QC is threatened; help"
                if (global.threedmode == false and (y - global.ballY < 96)) or (global.threedmode == true and (global.ballX - x < 96)) and distance_to_point(po_ox, po_oy) > 24{
            ...
            if (abs(instance_nearest(global.ballX, global.ballY, obj_player_teammate).x - global.ballX) + abs(instance_nearest(global.ballX, global.ballY, obj_player_teammate).y - global.ballY) < 24) and instance_nearest(global.ballX, global.ballY, obj_player_teammate).st_position < 4 and instance_nearest(global.ballX, global.ballY, obj_player_teammate).tackle == false and instance_nearest(x, y, obj_player_teammate).ch_off_broom == false{
        ...
        if instance_exists(obj_scuffle) == true{ //Own possession; am chaser evaluating scuffle
            ...
            thoughts = "Ignore scuffle; Retreat"
            if (global.threedmode == false){
                if obj_scuffle.y < 288 and point_distance(po_dx,po_dy,scuffle.x,scuffle.y) < 64{
                ...
                if obj_scuffle.x > 1024 and point_distance(po_dx,po_dy, obj_scuffle.x, obj_scuffle.y) < 96{
                    if(distance_to_point(obj_scuffle.x, obj_scuffle.y) > 16){
                        move_towards_point(obj_scuffle.x, obj_scuffle.y, moving_speed)
                        sprint = 1
                    }else{
        ...
        if instance_exists(obj_quaffle) and st_position < 3 and (instance_nearest(global.ballX, global.ballY, obj_player_dummy).x == x and instance_nearest(global.ballX, global.ballY, obj_player_dummy).y == y){
            sprint = 1
            move_towards_point(obj_quaffle.x, obj_quaffle.y, moving_speed)
            thoughts = "I am closest player to quaffle. Sprint on quaffle!!"
        }
    }
    if global.possession == 2 and st_position == 3{ //if quaffle in possession and you are a beater
        if ch_ball == true or po_noball == 1 or po_noball == 2{
            ...
            if (global.threedmode == false and y < global.ballY+16) or (global.threedmode == true and x > global.ballX-16) and distance_to_point(global.ballX, global.ballY) > 64{
                ...
                if po_withball == 2{ //attack
                    if global.threedmode == false{
                        move_towards_point(instance_nearest(global.ballX, global.ballY, obj_player_teammate).x, instance_nearest(global.ballX, global.ballY, obj_player_teammate).y, moving_speed)
                    }
            ...
            if ch_ball == true{ //patch, not pretty, redundant, etc.
                if instance_exists(obj_bludger) and instance_nearest(x, y, obj_player_dummy).st_position == 3 and instance_nearest(x, y, obj_player_dummy).ch_ball == false{
                    ...
                    thoughts = "Hover over bludger"
                        if distance_to_point(instance_nearest(x, y, obj_player_teammate).x, instance_nearest(x, y, obj_player_teammate).y) < 64{
                            move_towards_point(instance_nearest(x, y, obj_player_teammate).x, instance_nearest(x, y, obj_player_teammate).y, moving_speed)
                ...
                if distance_to_point(instance_nearest(global.ballX, global.ballY, obj_player_teammate).x, instance_nearest(global.ballX, global.ballY, obj_player_teammate).y) < 48 and distance_to_point(global.ballX, global.ballY) < 80 and instance_nearest(global.ballX, global.ballY, obj_player_teammate).ch_off_broom == false{
                    thoughts = "QC in danger; throw?"
                    if(windup < 8){
                        windup += 1
                    }
                    if round(random_range (1, 8)) == 1{
                        global.throw = st_throw-2
                        global.accuracy = st_accuracy
                        throw_leway = 15
                        with(instance_nearest(global.ballX, global.ballY, obj_player_teammate)){
                ...
                if(windup > 0){
                    windup -= 0.5 //if not thinking about throwing, lower ball
                }
            }
        }else{ //if ch_ball == false
            if instance_exists(obj_bludger){
                ...
                if po_noball == 2{
                    move_towards_point(instance_nearest(x, y, obj_team1bludgerwaypoint).x, instance_nearest(x, y, obj_team1bludgerwaypoint).y, moving_speed)
                    thoughts = "Move towards bludger"
                        if distance_to_point(obj_team1bludgerwaypoint.x, obj_team1bludgerwaypoint.y) < 32{
                            sprint = 1
                        }
    ...
    if global.possession == 4{ //Friendly keeper getting ball
        ...
        if st_position == 3 and ch_ball == true{ //if friendly keeper is getting ball and I have a bludger
            thoughts = "Cherry pick."
            if instance_exists(obj_player_test){
                if (global.threedmode == false and obj_player_test.y < room_height/2) or (global.threedmode == true and obj_player_test.x > room_width/2){
                    move_towards_point(obj_player_test.x, obj_player_test.y, moving_speed)
                }
            }else{
                if (global.threedmode == false and instance_nearest(x, y, obj_player_teammate).y < room_height/2) or (global.threedmode == true and instance_nearest(x, y, obj_player_teammate).x > room_width/2){
                    sprint = 1
                    move_towards_point((instance_nearest(x,y, obj_player_teammate)).x, (instance_nearest(x,y, obj_player_teammate)).y, moving_speed)
                }else{
                    if distance_to_point(po_dx, po_dy) > 4{
            ...
            if instance_exists(obj_player_teammate) and distance_to_object(obj_player_teammate) < 16 and instance_nearest(x, y, obj_player_teammate).ch_off_broom == false and ch_ball == true{
                ...
                if round(random_range(1, 8)) == 1{
                    ...
                    global.accuracy = st_accuracy
                    global.targetX = instance_nearest(x, y, obj_player_teammate).x
                    global.targetY = instance_nearest(x, y, obj_player_teammate).y 
                    global.targetSpeed = instance_nearest(x, y, obj_player_teammate).speed
                    global.targetDirection = instance_nearest(x, y, obj_player_teammate).speed
                    throw_leway = 15
                    if(scr_is_target_teammate_beat(global.targetX, global.targetY) == false){ 
                        scr_throw_ball(obj_bludger, x, y, 9, 0)
                        ch_ball = false
            ...
            if instance_exists(obj_player_test){
                if distance_to_object(obj_player_test) < 16{
                    if obj_player_test.ch_off_broom == false{
                        thoughts = "They're close; Throw bludger?"
                        if(windup < 8){
                            windup += 1
                        }
                        if round(random_range(1, 8)) == 1{
                            ...
                            global.accuracy = st_accuracy
                            throw_leway = 15
                            if(scr_is_target_teammate_beat(obj_player_test.x, obj_player_test.y) == false){ 
                                scr_throw_ball(obj_bludger, x, y, 3, 0)
                                ch_ball = false
                                exit
                            }
                        }
    ...
    if global.possession == 3{ //Opposing keeper getting ball
    ...
    if st_position == 3 and ch_ball == true{ //if quaffle free?
        if global.possession == 0{ //was changed from 1 to 0 11/24
            if instance_exists(obj_quaffle) == true{
                if distance_to_point(obj_quaffle.x, obj_quaffle.y) > 20 and distance_to_point(obj_quaffle.x, obj_quaffle.y) < 256{
                    ...
                    if distance_to_point(instance_nearest(global.ballX, global.ballY, obj_player_teammate).x, instance_nearest(global.ballX, global.ballY, obj_player_teammate).y) < 64 and distance_to_point(global.ballX, global.ballY) < 80{ // should also put player
                ...
                if distance_to_point(obj_center_hoop.x, obj_center_hoop.y+112) > 16{
                    move_towards_point(obj_center_hoop.x, obj_center_hoop.y+112, moving_speed)
                    thoughts = "Move below center hoop"
                }
                if distance_to_point(obj_center_hoop.x, obj_center_hoop.y+112) < 32{
                    move_towards_point(global.ballX, global.ballY, 0)
        ...
        if instance_exists(obj_player_test) == true and distance_to_point(global.ballX, global.ballY) < 160 and global.possession == 1{
            if obj_player_test.ch_ball == true{
                ...
                if round(random_range(1, 8)) == 1{
                    ...
                    throw_leway = 15
                    if(scr_is_target_teammate_beat(obj_player_test.x, obj_player_test.y) == false){ 
                        scr_throw_ball(obj_bludger, x, y, 3, 0) //throw at player based on which keys are held down
                        ch_ball = false
                        exit
                    }
                }
        ...
        if ((global.threedmode == false and (global.ballY < y + 32 and distance_to_point(global.ballX, global.ballY) < 160 or abs(global.ballY - obj_center_hoop.y) < 80))
        or(global.threedmode == true and (global.ballX > x - 32 and distance_to_point(global.ballX, global.ballY) < 160 or abs(global.ballX - obj_center_hoop.x) < 80)))
        and global.possession == 1 and ch_ball == true and instance_exists(obj_scuffle) == false{
            ...
            if round(random_range (1, 8)) == 1{
                ...
                global.accuracy = st_accuracy
                throw_leway = 15
                with(obj_player_test){ //scr_set_moving target makes sure the ONLY QUAFFLE CARRIER sets itself as the moving target, so this won't cause an error
        ...
        if instance_exists(obj_player_test) and distance_to_object(obj_player_test) < 16 and ch_ball == true and (global.possession != 1 and ((global.threedmode == false and global.ballY > room_height/2) or (global.threedmode == true and global.ballX < room_width/2))){
            if obj_player_test.ch_off_broom == false{
                thoughts = "They're close; Throw bludger?"
                if(windup < 8){
                    windup += 1
        ...
        if instance_exists(obj_player_teammate) and distance_to_object(obj_player_teammate) < 16 and instance_nearest(x, y, obj_player_teammate).ch_off_broom == false and ch_ball == true and global.possession != 3{
            thoughts = "They're close; Throw bludger?"
            if(windup < 8){
                windup += 1
            }
        ...
        if instance_exists(obj_quaffle) and distance_to_object(obj_quaffle) < 24 and ch_ball == true and global.possession != 2{
            if obj_quaffle.speed > 3{
                ...
                if round(random_range(1, 512)) <= st_accuracy{
                    ...
                    ch_ball = false
                    throw_leway = 15
                    scr_throw_ball(obj_bludger, x, y, 5, 0)
                    exit
                }
            }
        }
        if instance_exists(obj_scuffle){
            if distance_to_object(instance_nearest(obj_scuffle.x, obj_scuffle.y, obj_player_teammate)) < 24 and ch_ball == true{
                ...
                if round(random_range(1, 4)) == 1{
                    ...
                    global.accuracy = st_accuracy
                    throw_leway = 15
                    global.targetX = instance_nearest(obj_scuffle.x, obj_scuffle.y, obj_player_teammate).x
                    global.targetY = instance_nearest(obj_scuffle.x, obj_scuffle.y, obj_player_teammate).y+12
                    global.targetSpeed = 0
                    global.targetDirection = 0
                    if(scr_is_target_teammate_beat(global.targetX, global.targetY) == false){ 
                        scr_throw_ball(obj_bludger, x, y, 9, 0)
    ...
    if st_position == 3 and global.team2onseeker == 1 and instance_exists(obj_snitch) and obj_snitch.snitchonpitch == true{
        ...
        if ch_ball == true{
            if distance_to_point(obj_snitch.x, obj_snitch.y) < 64{
                sprint = 1
                if(scr_is_target_teammate_beat(global.seeker1x, global.seeker1y)) and distance_to_point(global.seeker1x, global.seeker1y) >= 32{ //if the other seeker isn't beat, move in front of them
                    thoughts = "opposing seeker not beat, move in between them at snitch"
                    move_towards_point((obj_snitch.x+global.seeker1x)/2, (obj_snitch.y+global.seeker1y)/2, moving_speed)
                }else if(distance_to_point(global.seeker1x, global.seeker1y) < 64) and (scr_is_target_dummy_beat(global.seeker1x, global.seeker1y) == false){ //if the other seeker isn't beat, and clsoe enougb, beat them
                    thoughts = "opposing seeker close, attack them"
                    move_towards_point(global.seeker1x, global.seeker1y, moving_speed)
                    if(distance_to_point(global.seeker1x, global.seeker1y)) < 24{
                        ...
                        if round(random_range(1, 4)) == 1{
                            ...
                            global.accuracy = st_accuracy
                            throw_leway = 15
                            global.targetX = instance_nearest(global.seeker1x, global.seeker1y, obj_player_teammate).x
                            global.targetY = instance_nearest(global.seeker1x, global.seeker1y, obj_player_teammate).y+12
                            //global.targetSpeed = instance_nearest(global.seeker1x, global.seeker1y, obj_player_teammate).x
                            //global.targetDirection = instance_nearest(global.seeker1x, global.seeker1y, obj_player_teammate).x
                            global.targetSpeed = 0
                            global.targetDirection = 0
                            if(scr_is_target_teammate_beat(global.targetX, global.targetY) == false){ 
                                scr_throw_ball(obj_bludger, x, y, 9, 0)
                    ...
                    move_towards_point(instance_nearest(global.seeker2x, global.seeker2y, obj_player_teammate).x, instance_nearest(global.seeker2x, global.seeker2y, obj_player_teammate).y, moving_speed)
                    if(distance_to_point(instance_nearest(global.seeker1x, global.seeker1y, obj_player_teammate).x, instance_nearest(global.seeker1x, global.seeker1y, obj_player_teammate).y) < 24){
                    ...
                    sprint = 0
                    if(distance_to_point((obj_snitch.x*4+instance_nearest(obj_snitch.x, obj_snitch.y, obj_baby_score_opp).x)/5,  (obj_snitch.y*4+instance_nearest(obj_snitch.x, obj_snitch.y, obj_baby_score_opp).y)/5) > 8){
                        move_towards_point((obj_snitch.x*4+instance_nearest(obj_snitch.x, obj_snitch.y, obj_baby_score_opp).x)/5, (obj_snitch.y*4+instance_nearest(obj_snitch.x, obj_snitch.y, obj_baby_score_opp).y)/5, moving_speed)
                    }else{
                        move_towards_point(instance_nearest(x, y, obj_player_teammate).x, instance_nearest(x, y, obj_player_teammate).y, 0)
                    }
                }
...
if drag == 1{ //I AM THE ONE WRAPPING
    if(instance_exists(obj_player_teammate)){
        if(distance_to_point(global.ballX, global.ballY)) > 3{
            move_towards_point(global.ballX, global.ballY, instance_nearest(x, y, obj_player_teammate).moving_speed)
        }else{
            move_towards_point(global.ballX, global.ballY, 0)
        }
    }else{
...
if(instance_exists(obj_player_teammate)){
    if(instance_exists(obj_quaffle)) or distance_to_object(obj_player_teammate) > 4{
        drag = 0;
    }
}
...
if tackle == true{
    ...
    if global.possession == 4 and st_position == 2{
        tackle = false
    }
...
/*if stiffarmed == true{
    if instance_exists(obj_player_test){
        move_towards_point(obj_player_test.x, obj_player_test.y, -1*((obj_player_test.st_power/5)+1))
    }
...
//if (argument == 0){
    if distance_to_object(obj_bludger) < 32 and round(random_range(1, 10-st_awareness)){
        if ch_ball == true and speed > 0{
            if((instance_nearest(x, y, obj_bludger).friendlyteam == 1)) and (abs(point_direction(x, y, instance_nearest(x, y, obj_bludger).x, instance_nearest(x, y, obj_bludger.y) - direction)) < 90) and (instance_nearest(x, y, obj_bludger).speed > 0){
                blocking = true
            }
        }
    }else{
...
if st_position == 3 and instance_exists(obj_bludger) and global.possession != -1{
    if ch_ball == false and scr_check_bludger_dummy() and scr_check_closest_beater_dummy() and obj_bludger.speed < 1
    {
        ch_immunity = true;
    }else{
        ch_immunity = false;
...
if st_position == 3 and distance_to_object(obj_team2bludgerwaypoint) > 4{
    ...
    if(obj_player_test.drag != 0 and drag == 0 and ch_ball == true){
        if(global.wasd == true){
            if(keyboard_check_direct(ord("A"))){ //LEFT
                if global.threedmode == false{
                    hspeed -= (obj_player_test.st_power*.1)
                }else{
                    hspeed -= (obj_player_test.st_power*.1)
                }
            }
            if(keyboard_check_direct(ord("W"))){ //UP
                if global.threedmode == false{
                    vspeed -= (obj_player_test.st_power*.1)
                }else{
                    vspeed -= (obj_player_test.st_power*.1)
                }
            }
