...
send_buffer2 = buffer_create(256, buffer_fixed, 1);
...
while(true){
    ...
    switch(message_id)
    {
        ...
        case PLAYER_INPUT:
            //show_message("Player input");
            var
            ww = buffer_read(buffer, buffer_u16);
            aa = buffer_read(buffer, buffer_u16);
            //ss = buffer_read(buffer, buffer_s16);
            //dd = buffer_read(buffer, buffer_s16);
            
            buffer_seek(send_buffer, buffer_seek_start, 0); //start back from the beginning of the 256 bytes
            buffer_write(send_buffer, buffer_u8, PLAYER_INPUT); //1 byte
            buffer_write(send_buffer, buffer_u16, client_id_current); //2 bytes
